...
const cli = path.join(skillRoot, 'bin/agentify.mjs');
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/agentify.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/agentify.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
